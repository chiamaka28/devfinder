    ...
    <div>
      {placeHolder && (
        <div className='flex justify-center items-center mt-[200px] font-extrabold text-2xl md:text-3xl'>
          <h2 className='text-black dark:text-white'>Look up a GitHub user</h2>
        </div>
      )}
      {error && (
        <div className='flex justify-center items-center mt-[200px] font-extrabold text-2xl md:text-3xl'>
      ...
      {getUser.map((user, index) => {
        return (
          <div
            key={index}
            className='bg-white dark:bg-navy-blue shadow-2xl px-5 p-3 mt-5 rounded-xl'
          >
            <div className='flex items-center gap-4 '>
              <div className=' '>
                <img
                  className='h-24 w-24 md:w-28 md:h-28 rounded-full '
                  src={user.avatar_url}
                  alt=''
                />
              </div>
              <div className='text-black dark:text-white md:flex md:items-start md:justify-between md:gap-32'>
                <div>
                  <p className='text-2xl '>{user.name}</p>
                  {user.login ? (
                    <p className='text-blue text-sm'>@{user.login}</p>
                  ) : (
                    ''
                  )}
                </div>
                <div>
                  <p className='text-sm md:text-base text-black dark:text-gray pt-1'>
            ...
            <div>
              {user.bio ? (
                <p className='dark:text-white text-gray p-3'>{user.bio}</p>
              ) : (
                <p className='text-navy-blue-2 dark:text-gray p-3'>
                  this user didnt add a bio
                </p>
              )}
            </div>
            <div className='text-black dark:text-white bg-whitish-blue  dark:bg-dark-navy-blue my-5 py-6 md:px-28 rounded-lg flex justify-center md:justify-between items-center gap-6'>
              <div className='text-center'>
                <h5 className='text-gray dark:text-white'>Repos</h5>
                <p className=' font-bold text-lg md:text-2xl'>
                  {user.public_repos}
                </p>
              </div>
              <div className='text-center'>
                <h5 className='text-gray dark:text-white'>Followers</h5>
                <p className=' font-bold text-lg md:text-2xl'>
                  {user.followers}
                </p>
              </div>
              <div className='text-center'>
                <h5 className='text-gray dark:text-white'>Following</h5>
                <p className=' font-bold text-lg md:text-2xl'>
                  {user.following}
                </p>
              </div>
            ...
            <div className='my-2 flex gap-3 items-center'>
              ...
              {user.location ? (
                <p className='dark:text-white text-gray'>{user.location}</p>
              ) : (
                <p className='text-navy-blue-2 dark:text-gray'>No Location</p>
              )}
            </div>
            <div className='my-2 flex gap-3 items-center'>
              <svg
              ...
              {user.blog ? (
                <a
                  href={user.blog}
                  target='_blank'
                  className='dark:text-white text-gray cursor-pointer'
                >
                  {user.blog}
                </a>
              ) : (
                <p className='text-navy-blue-2 dark:text-gray'>Not Available</p>
              )}
            </div>
            <div className='my-2 flex gap-3  items-center'>
              <svg
              ...
              {user.twitter_username ? (
                <p className='dark:text-white text-gray'>
                  {user.twitter_username}
                </p>
              ) : (
                <p className='text-navy-blue-2 dark:text-gray'>Not Available</p>
            ...
            <div className='my-2 flex gap-3 items-center'>
              ...
              {user.company ? (
                <p className='dark:text-white text-gray'>{user.company}</p>
              ) : (
                <p className='text-navy-blue-2 dark:text-gray'>Not Available</p>
              )}
            </div>
